...
def label_clusters(
    ...
    try:
        ...
        # Batch fetch all embedding indices grouped by cluster (avoids N+1 in loop)
        all_cluster_indices = conn.execute(
            f"SELECT {cluster_col} as cid, embedding_index FROM article_analysis WHERE {cluster_col} IS NOT NULL AND embedding_index IS NOT NULL"
        ).fetchall()
        indices_by_cluster: dict[int, list[int]] = {}
        for r in all_cluster_indices:
            indices_by_cluster.setdefault(r["cid"], []).append(r["embedding_index"])
        ...
        for row in clusters:
            ...
            # Use pre-fetched cluster indices
            idx_list = indices_by_cluster.get(cid, [])
            if not idx_list:
                continue
            ...
            if best_nid is not None:
                ...
                if dates is None:
                    ...
                    continue
                # Store centroid embedding and use its index
                centroid_idx = len(embeddings)
                embeddings = np.vstack([embeddings, new_centroid.reshape(1, -1)])
                conn.execute(
                    """UPDATE narratives
                       ...
                       WHERE id = ?""",
                    (dates[0], dates[0], dates[1], dates[1], centroid_idx, narrative_id),
                )
            else:
                ...
                cursor = conn.execute(
                    """INSERT INTO narratives (label, first_seen, last_seen, status, centroid_embedding_index)
                       VALUES (?, ?, ?, 'active', ?)""",
                    (label, dates[0], dates[1], centroid_idx),
                )
        ...
        # Single commit for all changes — atomic labeling
        # Save updated embeddings (with centroid vectors) before committing DB
